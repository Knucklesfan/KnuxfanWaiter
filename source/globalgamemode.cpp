...
GlobalGamemode::GlobalGamemode() {
    ...
    outer = new //cube definitions
        cube(
            glm::vec3(0.0f,0.0f,-1.935-(1.0/8.0)), //prepose
            glm::vec3(0.0f,0.0f,0.0f), //postpos
            glm::vec3((16.0000/9.0000),1.0f,(16.0000/9.0000)), //scale
    ...
    if(active) {
        ...
        switch(currentTransition.fade) {
            case FADE: {
                ...
                SDL_Rect splashbox = { 0, 0, 848, 480 };
                SDL_RenderFillRect(graphics::render, &splashbox);
                #else
                buffer.render(graphics::shaders[3],0,0,false);
                graphics::rect->render(graphics::shaders.at(1),{0,0},{848,480},0,{0,0,0,alpha},false,-1,{0,0,0,0});
                #endif
                break;
            }
            case BLOCKS: {
                ...
                if(fade) {
                    #ifdef __LEGACY_RENDER
                    SDL_Rect splashbox = { 0, -480+math::easeOutBounce((alpha*0.75))*480, 848, 480};
                    SDL_RenderFillRect(graphics::render, &splashbox);
                    #else
                    ...
                    // graphics::rect->render(graphics::shaders.at(1),{0,-480+(1-math::easeOutBounce(1-alpha))*480},{848,480},0,{0,0,0,1},false,-1,{0,0,0,0});
                    #endif
