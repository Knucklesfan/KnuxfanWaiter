...
void doGameLogic() {
    ...
    if(global->logic(graphics::deltaTime)) {
        gameplay::gamemode=global->currentTransition.gamemode%gameplay::gamemodes.size();
...
int main(int argc, char **argv) {
...
#ifdef __LEGACY_RENDER
    graphics::window = SDL_CreateWindow("Knuxfan's Tetriminos", 100, 100, 848, 480, SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
	...
	graphics::window = SDL_CreateWindow("SDLTetris Rewritten", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 848, 480, SDL_WINDOW_OPENGL | SDL_WINDOW_SHOWN | SDL_WINDOW_RESIZABLE);
    if (!graphics::window) {
    ...
    SDL_RenderSetLogicalSize(graphics::render, 848, 480);
#else
    ...
    #ifdef __LEGACY_RENDER
    SDL_Texture* rendertext = SDL_CreateTexture(graphics::render, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_TARGET, 848,480);
